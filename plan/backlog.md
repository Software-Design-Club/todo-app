...
- [x] Unify invite flow: searching for an existing app user should send them an email invitation link rather than adding them directly as a collaborator — the email invite flow works regardless of whether the invitee has an account, so the user-search-and-add path should route through the same invitation workflow
- [x] Use inline banner for InviteByEmailForm feedback inside the ManageCollaborators dropdown: add `onError?: (message: string) => void` prop to `InviteByEmailForm`; when provided, call `onError(msg)` instead of `toast.error()`; suppress `toast.success()` when `onSuccess` is provided so the parent owns all feedback. Pass `onError={(msg) => setError(msg)}` and call `setSuccessMessage(...)` inside the existing `onSuccess` wrapper in `ManageCollaborators`. The collaborators page passes neither prop and keeps existing toast behavior.
- [ ] Refresh `/lists/collaborators` page after any invitation action (revoke, resend, copy link, approve, reject) — currently only invite triggers an update; revoke and other actions leave the page stale. Add `router.refresh()` or `revalidatePath` calls consistently across all action handlers on that page.
- [ ] Add the search-and-invite form (the "Invite New Collaborators" panel from the ManageCollaborators dropdown) to the `/lists/collaborators` page so owners can search for and invite users without navigating back to the list.
