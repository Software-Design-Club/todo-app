...
- [ ] Use inline banner for InviteByEmailForm feedback inside the ManageCollaborators dropdown: add `onError?: (message: string) => void` prop to `InviteByEmailForm`; when provided, call `onError(msg)` instead of `toast.error()`; suppress `toast.success()` when `onSuccess` is provided so the parent owns all feedback. Pass `onError={(msg) => setError(msg)}` and call `setSuccessMessage(...)` inside the existing `onSuccess` wrapper in `ManageCollaborators`. The collaborators page passes neither prop and keeps existing toast behavior.
