...
const markInvitationEmailDeliveryByProviderIdMock = vi.fn();
...
vi.mock("@/lib/invitations/service", () => ({
  markInvitationEmailDeliveryByProviderId:
    markInvitationEmailDeliveryByProviderIdMock,
}));
...
describe("resend webhook route", () => {
  ...
  it("returns 401 when signature verification fails", async () => {
    ...
    expect(markInvitationEmailDeliveryByProviderIdMock).not.toHaveBeenCalled();
  });
  ...
  it("accepts valid Svix payloads and persists delivery failure metadata", async () => {
    ...
    markInvitationEmailDeliveryByProviderIdMock.mockResolvedValueOnce(null);
    ...
    expect(markInvitationEmailDeliveryByProviderIdMock).toHaveBeenCalledWith({
